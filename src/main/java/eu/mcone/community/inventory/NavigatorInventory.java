...
public class NavigatorInventory extends CoreInventory {
    ...
    public NavigatorInventory(Player p) {
        ...
        if (p.hasPermission("community.bypass.entrance") || gamePlayer.hasDefaultItem(DefaultItem.FESTIVAL_ENTRANCE) || FreeEntranceCMD.freeEntrance) {
            setItem(InventorySlot.ROW_2_SLOT_3, new ItemBuilder(Material.FIREWORK_CHARGE, 1, 0).displayName(Transl.get("community.stage.item.stage.display", cp)).lore(Transl.get("system.teleport.lore", cp)).create(), e -> {
            ...
            setItem(InventorySlot.ROW_2_SLOT_3, new ItemBuilder(Material.FIREWORK_CHARGE, 1, 0).displayName(Transl.get("community.stage.item.ticket.display", cp)).lore(Transl.get("system.teleport.lore", cp)).create(), e -> {
                p.closeInventory();
        ...
        setItem(InventorySlot.ROW_2_SLOT_7, new ItemBuilder(Material.NETHER_STAR, 1, 0).displayName(Transl.get("community.stage.item.spawn.display", cp)).lore(Transl.get("system.teleport.lore", cp)).create(), e -> {
